 ...
 *  - Atalhos para abrir a mesma view do iqar-trigger via botões de info.
 * ===========================================
 */
...
/**
 * Botões de info (MP10/MP2.5) abrindo a MESMA view inline do iqar-trigger.
 * Dispara o mesmo listener de click do #iqar-trigger (vertical.js),
 * evitando submit se os botões estiverem dentro de <form>.
 */
document.addEventListener("DOMContentLoaded", () => {
  const fireSameAsIqar = (btnId) => {
    const btn  = document.getElementById(btnId);
    const main = document.getElementById("iqar-trigger");
    if (!btn || !main) return;

    // Evita submit se for <button> sem type
    ...
    btn.addEventListener("click", (ev) => {
      ev.preventDefault();
      ev.stopPropagation();
      // Dispara o MESMO click que o vertical.js escuta no iqar-trigger
      main.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
    });
  };

  fireSameAsIqar("iqar-info-trigger-mp10");
  fireSameAsIqar("iqar-info-trigger-mp25");
});
